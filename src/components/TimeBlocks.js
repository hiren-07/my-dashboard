...
export default function TimeBlocks({ blocks, setBlocks, theme }) {
  ...
  function handleDrop(e, dropIdx) {
    ...
    setBlocks(bs => {
      const newBs = [...bs];
      const draggedItem = newBs[draggedIdx];
      newBs.splice(draggedIdx, 1);
      newBs.splice(dropIdx, 0, draggedItem);
      return newBs;
    });
    setDraggedIdx(null);
  }
        ...
        {blocks.map((b, idx) => {
          ...
          return (
            <div
              ...
              style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12, position: "relative", cursor: draggedIdx === idx ? "grabbing" : "grab", opacity: draggedIdx === idx ? 0.4 : 1, transition: "opacity 0.2s" }}
            >
              <div style={{ position: "absolute", left: -22, top: "50%", transform: "translateY(-50%)", width: 12, height: 12, borderRadius: "50%", background: b.color, border: `2px solid ${isActive ? b.color : theme.border2}`, boxShadow: isActive ? `0 0 10px ${b.color}99` : "none", transition: "all 0.4s" }} />
              <div style={{ flex: 1, background: isActive ? b.color + "11" : isPast ? theme.topbar : theme.card, border: `1.5px solid ${isActive ? b.color + "55" : theme.border}`, borderRadius: 12, padding: "12px 15px", display: "flex", alignItems: "center", gap: 12, opacity: isPast && b.done ? 0.45 : 1, transition: "all 0.3s" }}>
                <div style={{ color: theme.muted, cursor: "grab", fontSize: 16, marginRight: 2 }}>⋮⋮</div>
                <div style={{ flex: 1 }}>
